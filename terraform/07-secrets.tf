...
resource "google_secret_manager_secret" "db_password" {
  secret_id = "${var.project_application_name}-db_password"
  project   = var.gcp_project
  replication {
    auto {}
  }
}
resource "google_secret_manager_secret" "nginx_password" {
  secret_id = "${var.project_application_name}-nginx_password"
  project   = var.gcp_project
  replication {
    auto {}
  }
...
resource "google_secret_manager_secret" "argocd_password" {
  secret_id = "${var.project_application_name}-argocd_password"
  project   = var.gcp_project
  replication {
    auto {}
  }
...
resource "google_secret_manager_secret_version" "nginx_password" {
  secret      = google_secret_manager_secret.nginx_password.id
  secret_data = random_password.nginx_password.result
}
resource "google_secret_manager_secret_version" "argocd_password" {
  secret      = google_secret_manager_secret.argocd_password.id
  secret_data = random_password.argocd_password.result
}
